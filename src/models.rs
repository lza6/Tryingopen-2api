...
pub fn catalog() -> Vec<ModelMeta> {
    ...
    m!("qwen/qwen3.8-27b", "Qwen3.8 27B", "Alibaba", "262k", 3.2, true, true);
    m!("nvidia/nemotron-3.5-lightning", "Nemotron 3.5 Lightning", "NVIDIA", "262k", 0.25, true, false);
    m!("deepseek/deepseek-v4-flash-0731", "DeepSeek V4 Flash", "DeepSeek", "1M", 0.18, true, false);
    m!("deepseek/deepseek-v4-pro-0813", "DeepSeek V4 Pro", "DeepSeek", "1M", 1.98, true, false);
    m!("google/gemma-4-31b-it", "Gemma 4 31B", "Google", "256k", 0.4, true, true);
    m!("google/gemma-4-26b-a4b-it", "Gemma 4 26B", "Google", "256k", 0.4, true, true);
    m!("openai/gpt-oss-120b", "GPT-OSS 120B", "OpenAI", "128k", 0.6, true, false);
    m!("meta/muse-glimmer-30b", "Muse Glimmer 30B", "Meta", "131k", 1.5, true, false);
    m!("moonshotai/kimi-k3", "Kimi K3", "Moonshot AI", "1M", 15.0, true, true);
    m!("minimax/minimax-m3", "MiniMax M3", "MiniMax", "1M", 1.2, true, true);
    m!("thinkingmachines/inkling-small", "Inkling Small", "ThinkingMachines", "524k", 1.2, true, true);
    m!("z-ai/glm-5.2", "GLM 5.2", "Zhipu AI", "1M", 1.54, true, false);
    v
}
...
pub fn parse_ctx(ctx: &str) -> i64 {
    ...
    num.parse::<f64>().map(|n| (n * unit as f64) as i64).unwrap_or(0)
}
...
impl ModelRegistry {
    ...
    pub async fn normalize(&self, requested: &str) -> String {
        ...
        let matched = list.iter().find_map(|m| {
            if let Some(surface) = m.id.rsplit('/').next() {
                if surface == r { return Some(m.id.clone()); }
            }
            if m.label.eq_ignore_ascii_case(r) { return Some(m.id.clone()); }
            None
        });
    ...
    pub async fn resolve(&self, requested: &str) -> String {
        ...
        if self.has_model(&norm).await { return norm; }
        for fb in ["deepseek/deepseek-v4-flash-0731", "z-ai/glm-5.2", "minimax/minimax-m3"] {
            if self.has_model(fb).await { return fb.to_string(); }
        }
    ...
    pub async fn replace_from_parsed(&self, records: Vec<ModelMeta>) -> usize {
        if records.is_empty() { return self.inner.read().await.len(); }
...
pub fn openai_models(list: &[ModelMeta]) -> Vec<OpenAIModelObject> {
    list.iter().map(|m| OpenAIModelObject {
        id: m.id.clone(),
        object: "model".into(),
        created: 0,
...
pub fn anthropic_models(list: &[ModelMeta]) -> Vec<AnthropicModelObject> {
    list.iter().map(|m| AnthropicModelObject {
        id: m.id.clone(),
        name: m.label.clone(),
        created: 0,
        input_modalities: if m.vision { vec!["text".into(), "image".into()] } else { vec!["text".into()] },
